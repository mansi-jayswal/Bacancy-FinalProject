...
function Section() {
  ...
  const handleNavigate = (cuisinename) =>{
      ...
      <div className="flex w-full  m-2 p-4 rounded justify-center">
        {cuisines.map((cuisine, index) => (
          <div key={index} className="flex items-center flex-col mr-8">
              {/* <Link to={`/recipes?cuisine=${cuisine.name}`} /> */}
            ...
            onClick={()=>{handleNavigate(cuisine.name)}}>
              <img
                src={cuisine.image}
                alt={cuisine.name}
              ...
              <p className="absolute bottom-7 left-0 right-0 text-center text-white font-bold z-10">
                {cuisine.name}
              </p>
            </div>
          </div>
